'''
Pulled from NotSoBot, rewritten to work in Discord.py rewrite.
'''
  ...
  async def chan(self, ctx, board:str=None):
    ...
    try:
      ...
      if board is None:
        ...
        msg = discord.Embed(
          title=f"Random post from /{board}/",
          description=post,
          color=discord.Color.dark_blue()
        )
        await ctx.send(embed=msg)
    except Exception as e:
      await error.Error(self.bot, "Invalid Board!", "Please select a valid board. Example: g").send(ctx)
